...
verificaClafMaiorQueXAux([]).
verificaClafMaiorQueXAux([(IdEnc,Nota,_,_,_,_)]) :- encEntregueAtraso(IdEnc), encomenda(IdEnc,_,_,_,Prazo,DataI,DataF), estafetaPenalizacao(DataI,DataF,Prazo,P), Nota =< P.
verificaClafMaiorQueXAux([(IdEnc,_,_,_,_,_)]) :- nao(encEntregueAtraso(IdEnc)).
verificaClafMaiorQueX([(IdEnc,Nota,_,_,_,_) | T]) :-
    (encEntregueAtraso(IdEnc) -> 
        (encomenda(IdEnc,_,_,_,Prazo,DataI,DataF),estafetaPenalizacao(DataI,DataF,Prazo,P), Nota =< P -> verificaClafMaiorQueXAux(T); fail); 
    verificaClafMaiorQueXAux(T)).
